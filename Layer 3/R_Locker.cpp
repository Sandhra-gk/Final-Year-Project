...
void InitializeLog() {
    ...
    logstream.open((std::to_wstring(st.wDay) + L"." + std::to_wstring(st.wMonth) + L"." + std::to_wstring(st.wYear) + L".txt").c_str());
...
DWORD WINAPI NotifyUser(LPVOID param) {
    ...
    *size = 1024;
    QueryFullProcessImageNameW(hProcess, 0, buffer, size);
...
void WalkDirs(std::wstring dir_name) {
    ...
    if (hTrap == INVALID_HANDLE_VALUE) {
        CreateSymbolicLinkW(trap_name.c_str(), trap_target.c_str(), 0x0);
        SetFileAttributesW(trap_name.c_str(), FILE_ATTRIBUTE_HIDDEN);
...
std::wstring GetMainDrive() {
    wchar_t* buffer = new wchar_t[1024];
    int error;
    error = GetEnvironmentVariableW(L"SystemDrive", buffer, 1024);
...
void PopulateTraps() {
    ...
    wchar_t* buffer = new wchar_t[1024];
    int error;
    error = GetEnvironmentVariableW(L"userprofile", buffer, 1024);
    if (error == 0) {
...
double CalculateEntropy(const std::wstring& filePath) {
    ...
    while (file.read(reinterpret_cast<wchar_t*>(&byte), sizeof(byte))) {
        byteFrequency[byte]++;
        totalBytes++;
    }
            ...
            if (rootData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
                if (next_dir.compare(L".") != 0 && next_dir.compare(L"..") != 0) {
                    ...
                    do {
                        ...
                        // if (GetBinaryTypeW(exePath.c_str(), lpBinaryType)) {
                        //     whiteList[exePath] = true;
                        // }

                        // Calculate entropy
                        double entropy = CalculateEntropy(exePath);
                        const double entropyThreshold = 3.7; // Adjust this threshold as needed

                        if (entropy < entropyThreshold) {
                            whiteList[exePath] = true;
                            std::wcout << L"Added to whitelist (low entropy): " << exePath << std::endl;
                        }
                        else {
                            whiteList[exePath] = true;
                            std::wcout << L"Ignored (High entropy): " << exePath << std::endl;
                        }
...
int main(int argc, char* argv[]){
    ...
    // for (const auto& trapPath : trap_paths) {
    // CheckFileEntropy(trapPath);
    // }

    // Populate white list
    std::vector<std::wstring> roots;
    std::wstring mainDrive(GetMainDrive());
    roots.push_back(mainDrive + L"\\Ransomware");
    ...
    // std::wstring testFolderPath = L"C:\\Users\\sandh\\Downloads\\R Locker Test\\Test";
    // if (!EnumDirs(testFolderPath)) {
    //     std::wcerr << L"Failed to enumerate files in test folder." << std::endl;
    //     return -1;
    // }
